...
inline,rewrite :: (PP.Pretty f, Ord f, Typeable f) => String -> Inline.Selector f Int -> Strategy (RewriteSystem f) (RewriteSystem f)
inline n f = processor (Inline InlineFull (InlineSelector n f))
rewrite n f = processor (Inline InlineRewrite  (InlineSelector n f))

usableRules, usableRulesFast :: (PP.Pretty f, Ord f, Typeable f) => Strategy (RewriteSystem f) (RewriteSystem f)
usableRules = processor (UsableRules DFA)
usableRulesFast = processor (UsableRules Syntactic)

dfa :: (Show f, PP.Pretty f, Ord f, Typeable f) => String -> Instantiate.RefineP f -> Strategy (RewriteSystem f) (RewriteSystem f)
dfa n f = processor (CFA (CFARefinement n f))

compress :: (PP.Pretty f, Ord f, Typeable f) => Strategy (RewriteSystem f) (RewriteSystem f)
compress = processor Compression

toTctProblem :: (Show f, PP.Pretty f, Ord f, Typeable f) => Strategy (RewriteSystem f) TrsProblem
...
cfa :: (Show f, PP.Pretty f, Ord f, Typeable f) => Strategy (RewriteSystem f) (RewriteSystem f)
cfa = dfa "control flow analysis" (Instantiate.refineND `Instantiate.orRefine` Instantiate.refineHOVars)

simplifyTRS :: (Show f, PP.Pretty f, Ord f, Typeable f) => Strategy (RewriteSystem f) (RewriteSystem f)
simplifyTRS =
